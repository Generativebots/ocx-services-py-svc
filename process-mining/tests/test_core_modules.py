"""
Process Mining — Unit tests for Miner, Conformance, and Bottleneck modules.

Tests the core classes directly (not via API) for coverage boost.
"""
...
# Ensure process-mining package is importable
...
class TestBottleneckAnalyzer:
    ...
    def test_transition_performance(self):
        ba = BottleneckAnalyzer()
        base = datetime(2026, 1, 1, tzinfo=timezone.utc)
        # Create trace with known wait times
        trace = Trace(
            trace_id="tw1",
            tenant_id="t-1",
            activities=[
        ...
        report = ba.analyze([trace])
        # Should detect transition wait time
